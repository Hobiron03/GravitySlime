namespace UntitledGames.Transforms
{
    ...
    public partial class TransformProEditor
    {
        ...
        private static bool CanGenerateCollider()
        {
            ...
            {
                bool hasColliderSize = TransformProEditor.Selected.Any(x => (x.ColliderBounds != null) && (x.ColliderBounds.Local.size != Vector3.zero));
                bool hasRendererSize = TransformProEditor.Selected.Any(x => (x.RendererBounds != null) && (x.RendererBounds.Local.size != Vector3.zero));
        ...
        public void PositionField()
        {
            ...
            GUILayout.Label(TransformProStyles.Icons.PositionSmall, TransformProStyles.LabelSmallIcon, GUILayout.Width(16), GUILayout.Height(16));
            Rect toggleRect = GUILayoutUtility.GetLastRect();
            toggleRect.width = 20;
            TransformProPreferences.AdvancedPosition = EditorGUI.Foldout(toggleRect, TransformProPreferences.AdvancedPosition, GUIContent.none);
        ...
        private void DrawPositionAdvancedPanel()
        {
            ...
            GUIContent dropContent = new GUIContent(TransformProStyles.Icons.Drop, TransformProStrings.SystemLanguage.TooltipDrop);
            if (GUILayout.Button(dropContent, TransformProStyles.Buttons.Icon.Left))
            {
                int failed = TransformProEditor.Drop();
                if (failed > 0)
                {
                    Debug.LogWarning(string.Format("[<color=red>TransformPro</color>] {0} drop operations failed.\nCould not find any colliders underneath the transform{1}.",
                                                   failed,
                                                   failed > 1 ? "s" : ""));
                }
            }
            GUIContent groundContent = new GUIContent(TransformProStyles.Icons.Ground, TransformProStrings.SystemLanguage.TooltipGround);
            if (GUILayout.Button(groundContent, TransformProStyles.Buttons.Icon.Right))
            {
                int failed = TransformProEditor.Ground();
                if (failed > 0)
                {
                    Debug.LogWarning(string.Format("[<color=red>TransformPro</color>] {0} ground operations failed.\nCould not find colliders underneath the transform{1}.",
                                                   failed,
                                                   failed > 1 ? "s" : ""));
                }
            }
            ...
            GUIContent createBoxLabel = new GUIContent(TransformProStyles.Icons.ColliderBox, TransformProStrings.SystemLanguage.CreateBoxCollider);
            ...
            GUIContent createCapsuleLabel = new GUIContent(TransformProStyles.Icons.ColliderCapsule, TransformProStrings.SystemLanguage.CreateCapsuleCollider);
